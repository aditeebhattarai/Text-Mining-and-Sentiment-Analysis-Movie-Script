...
scripts <- scripts %>% mutate(Speaker_lower = tolower(Speaker))
...
head(scripts)
#Rename Lockgart
#scripts <-scripts %>% mutate(Speaker_noTypos = gsub('harri','harry',Dialogue_noPunctuation))
#head(scripts)
...
scripts <- scripts %>% mutate(Dialogue_noSpaces = gsub('\\s+',' ',Dialogue_noTypos))
head(scripts)
...
########### stemming ###########
################################
scripts <-scripts %>% mutate(Dialogue_Stem = stem_strings(Dialogue_noSpaces))
head(scripts)
...
#CSV Export ##########
write.csv(my_text,"try.csv")

#str(my_text)

#Read csv
script2<-read.csv("try.csv")
...
script2 <- read.csv("HPWorkingFile.csv")
(script2 <- script2 %>% select(Speaker,Listener))
...
barplot(dtm_d[1:5,]$freq, las = 2, names.arg = dtm_d[1:5,]$word,
        col ="aquamarine2", main ="Top 5 most frequent words",
...
tokens <- tokens %>% select(Speaker_lower, Listener_lower, word)
...
ggplot(my_text$Dialogue_Lemma=sentiment, aes(x=reorder(sentiment, -n, sum), y=n))+ 
...
tokens <- tokens %>% select(Dialogue_lower, word)
...
Sentiment %>% 
  ...
  scale_x_discrete(limits = c("HARRY","HERMOINE","RON","DRACO","DOBBY","DUMBLEDORE","LOCKHART","TOM RIDDLE"))+
